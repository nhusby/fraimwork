...
import { createModelConfig, MODELS } from "../models.js";
...
export class FastAgent extends FailoverAgent {
  ...
  async initializeModels(modelConfigs: any) {
    const modelStats = await this.fetchModelStats(modelConfigs);
  ...
  private async fetchModelStats(modelConfigs: any): Promise<any[]> {
    const statsPromises = modelConfigs.map(async (config: any) => {
      const name = config.name;
      if ("slug" in config) {
        const variant = name.endsWith(":free") ? "&variant=free" : "";
        const url = `https://openrouter.ai/api/frontend/stats/endpoint?permaslug=${config.slug}${variant}`;

        try {
          const response = await axios.get(url);
          return { name, ...response.data.data[0].stats };
        } catch (error) {
          console.error(`Failed to fetch stats for ${name}:`, error);
          return { name, avg_latency: Infinity }; // Penalize failed fetches
        }
      }
      return { name, avg_latency: Infinity }; // Models without slugs are not included
    });
